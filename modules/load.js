...
// Checks if the map file is from a version of the fork
function isSNFMG(version) {
  const snfmg_versions = ["SNFMG.1.63", "SNFMG.1.64", "SNFMG.1.65", "SNFMG.1.651", "SNFMG.1.652", "SNFMG.1.66"];
  for (const i in snfmg_versions) {
    if (snfmg_versions[i] === version) {
      return true;
    }
  }
  return false;
...
// Checks if the map file is valid and from a compatible version of FMG
...
function uploadMap(file, callback) {
  ...
  fileReader.onload = function (fileLoadedEvent) {
    ...
    let message = "",
      load = false;
    if (isSNFMG(version) || isValidMap(parsed)) {
      load = true;
...
function parseLoadedData(data) {
  try {
    ...
    void (function resolveVersionConflicts() {
      ...
      if (version < 1.64 || version === "SNFMG.1.63") {
      ...
      if (version < 1.65 || version === "SNFMG.1.64") {
      ...
      if (version < 1.652 || version === "SNFMG.1.65" || version === "SNFMG.1.651") {
